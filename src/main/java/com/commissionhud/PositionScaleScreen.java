...
public class PositionScaleScreen extends Screen {
    ...
    // Dragging state
    private enum DragTarget { NONE, COMMISSION, POWDER }
    ...
    protected void init() {
        ...
        addDrawableChild(new SliderWidget(leftX - sliderWidth / 2, height - 75, sliderWidth, buttonHeight,
            Text.literal("Commission Scale: " + String.format("%.1f", cfg.scale)), (cfg.scale - 0.5) / 1.5) {
            @Override
            protected void updateMessage() {
                cfg.scale = (float) (0.5 + value * 1.5);
                setMessage(Text.literal("Commission Scale: " + String.format("%.1f", cfg.scale)));
            }
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal("Reset Powder Pos"), button -> {
